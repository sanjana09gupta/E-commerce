...
interface NewArrivalCardsProps {
    title: string;
    unit: string;
    imageUrl: string;
    price: string;
  ...
  const NewArrival: React.FC<NewArrivalCardsProps> = ({
    title,
    unit,
    imageUrl,
    price,
      ...
      >
        <div className="bg-white p-5 rounded-br-[50px] rounded-tl-[50px] sm:p-3">
          ...
          <div className="flex items-center gap-3 sm:!gap-1.5">
            ...
            {cancelprice && (
              <p className="text-gray-400 line-through text-[12px] font-normal sm:text-[10px]">{cancelprice}</p>
            )}
          </div>
